...
description: >
  ...
  Covers: creating new xlsx from scratch, reading and analyzing existing files, editing existing
  xlsx with zero format loss, formula recalculation and validation, and applying professional
  financial formatting standards.
  TRIGGER when: user asks to create/build/modify/analyze Excel spreadsheet, financial model, pivot table, or tabular data
  SKIP when: user needs PDF (use minimax-pdf), Word document (use minimax-docx), or PowerPoint (use pptx-generator)
license: MIT
metadata:
  version: "1.0"
